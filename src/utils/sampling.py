...
def sample_trajectories(dataset, n_episodes: int = 2, ep_len: int = None):
    """
    Returns a list of n_episodes state arrays.

    Args:
        dataset: An OGBench dataset dict with 'observations' and 'terminals' keys.
        n_episodes: Number of episodes to return.
        ep_len: Maximum episode length (exclusive). If None, no length filter is applied.
    """
    observations = dataset['observations']
    terminals = dataset['terminals']
    ...
    for i in np.random.permutation(len(end_indices)):
        ...
        if ep_len is None or len(ep_obs) < ep_len:
            trajs.append(ep_obs)
